...
func NewConnection(provider Provider, secrets ConnectionSecrets) Connection {
	return Connection{
		provider: provider,
		secrets:  secrets,
	}
...
func (c *Connection) GenerateRepositoryToken(_ context.Context, repo *provisioning.Repository) (*connection.ExpirableSecureValue, error) {
	...
	if repo.Spec.Type != c.provider.RepositoryType() {
		return nil, fmt.Errorf("repository type %q is not served by this connection (serves %q)", repo.Spec.Type, c.provider.RepositoryType())
	}
...
// TokenState reports the state of the stored token. A missing expiry means the
// provider issued a non-expiring access token (e.g. GitHub OAuth apps).
func (c *Connection) TokenState() (bool, time.Time) {
	token, err := parseToken(c.secrets.Token)
	if err != nil {
