...
func TestConnection_Test(t *testing.T) {
			...
			expectedErrors: []provisioning.ErrorDetails{
				{
					...
					Field:  "secure.token",
					Detail: "the connection has not been authorized yet",
				},
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			conn := oauth.NewConnection(provider, oauth.ConnectionSecrets{Token: tt.token})

			results, err := conn.Test(context.Background())
...
func TestConnection_GenerateRepositoryToken(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			conn := oauth.NewConnection(newMockProvider(t, ""), oauth.ConnectionSecrets{Token: tt.token})

			value, err := conn.GenerateRepositoryToken(context.Background(), tt.repo)
			if tt.expectedErr != "" {
...
func TestConnection_ListRepositories(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		conn := oauth.NewConnection(provider, oauth.ConnectionSecrets{
			Token: marshalTestToken(t, &oauth2.Token{AccessToken: "access"}),
		})

		result, err := conn.ListRepositories(context.Background())
		require.NoError(t, err)
		assert.Equal(t, repos, result)
	})

	t.Run("failure - no token stored", func(t *testing.T) {
		conn := oauth.NewConnection(newMockProvider(t, ""), oauth.ConnectionSecrets{})
...
func TestConnection_GenerateConnectionToken(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			conn := oauth.NewConnection(newMockProvider(t, srv.URL), oauth.ConnectionSecrets{
				ClientSecret: "client-secret",
				Token:        tt.token,
			})

			raw, err := conn.GenerateConnectionToken(context.Background())
			if tt.expectedErr != "" {
				require.ErrorContains(t, err, tt.expectedErr)
				return
			}
...
func TestConnection_ExchangeAuthorizationCode(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			raw, err := conn.ExchangeAuthorizationCode(context.Background(), tt.code, "https://grafana.example/callback")
			if tt.expectedErr != "" {
				require.ErrorContains(t, err, tt.expectedErr)
				return
			}
...
func TestConnection_TokenState(t *testing.T) {
	expiry := time.Now().Add(time.Hour).Truncate(time.Second)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			hasToken, tokenExpiry := conn.TokenState()
			assert.Equal(t, tt.expectHasToken, hasToken)
...
func newMockProvider(t *testing.T, tokenURL string) *oauth.MockProvider {
	...
	provider.EXPECT().RepositoryType().Return(provisioning.GitLabRepositoryType).Maybe()
	provider.EXPECT().ClientID().Return("client-id").Maybe()
