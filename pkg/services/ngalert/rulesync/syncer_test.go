...
func (f *fakeRuleService) GetAlertGroupsWithFolderFullpath(_ context.Context, _ identity.Requester, filterOpts *provisioning.FilterOptions) ([]models.AlertRuleGroupWithFolderFullpath, error) {
	...
	// real query the syncer now relies on for folder-scoped prune.
...
func (f *fakeRuleService) GetAlertRules(_ context.Context, _ identity.Requester) ([]*models.AlertRule, map[string]models.Provenance, error) {
	return f.rules, f.provenances, nil
}
...
func TestSyncOrg_Promote(t *testing.T) {
	...
	rs := &fakeRuleService{
		rules: []*models.AlertRule{{
			...
			Title:           "r",
			OrgID:           1,
			NamespaceUID:    "folder-ns1",
			RuleGroup:       "g1",
			IntervalSeconds: 60,
		}},
		provenances: map[string]models.Provenance{
			"rule-1": models.ProvenanceConvertedPrometheus,
		},
	}
	fetch := &fakeFetcher{cfg: upstreamGroup("g1", "A"), hash: 5}
	s := newTestSyncer(t, cs, fetch, rs)
	// The converted rule lives under one of the sync's namespace subfolders.
	s.namespaceStore = fakeNamespaceStore{children: []*folder.FolderReference{{UID: "folder-ns1", Title: "ns1"}}}

	s.SyncOrg(context.Background(), 1)

	// Promotion rewrites the converted rule group with provenance=none, and does
	// NOT fetch upstream or prune.
...
func TestSyncOrg_PromoteIdempotentWhenNothingOwned(t *testing.T) {
	cs := &fakeConfigStore{uid: "ds1", promote: true}
	// A rule under the sync folder that is already provenance=none (promoted) →
