...
// configured for an org and whether a given folder is part of the sync's
// managed folder subtree. When sync is configured, the convert API rejects
// manual rule imports that target the sync-managed folder, since the sync
// worker owns those converted rules and would overwrite/prune them; imports
// into unrelated folders are still allowed. Satisfied by
// *rulesync.ExternalRulerSyncer; may be nil (sync disabled / test paths) —
...
func (srv *ConvertPrometheusSrv) RouteConvertPrometheusPostRuleGroups(c *contextmodel.ReqContext, promNamespaces map[string][]apimodels.PrometheusRuleGroup) response.Response {
	...
	// org AND the import targets the sync-managed folder subtree. The gate is
	// folder-scoped: the sync worker owns the converted rules under its dedicated
	// folder and would overwrite/prune them, but imports into unrelated folders
	// are allowed.
	if srv.rulerSync != nil {
		...
		if syncConfigured {
			managed, err := srv.rulerSync.IsManagedFolder(c.Req.Context(), c.GetOrgID(), workingFolderUID)
			if err != nil {
