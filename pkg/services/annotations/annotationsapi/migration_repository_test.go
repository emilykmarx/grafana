...
func TestMigrationRepository_Delete(t *testing.T) {
	...
	t.Run("mass delete without a dashboard UID clears legacy and skips the new store", func(t *testing.T) {
		legacy := &fakeLegacy{}
		proxy := &fakeProxy{}
		repo := newTestRepo(t, "proxy-writes", legacy, proxy, usertest.NewUserServiceFake())
	...
	t.Run("mass delete without a dashboard UID errors in proxy-all", func(t *testing.T) {
		legacy := &fakeLegacy{}
		proxy := &fakeProxy{}
		repo := newTestRepo(t, "proxy-all", legacy, proxy, usertest.NewUserServiceFake())
		...
		// nolint: staticcheck
		params := &annotations.DeleteParams{OrgID: 1, DashboardID: 9, PanelID: 2}
		require.ErrorIs(t, repo.Delete(context.Background(), params), errMassDeleteNeedsUID)

		assert.Empty(t, proxy.massDeleteCalls)
		require.Len(t, legacy.deleteParams, 1, "legacy copies are still removed")
	})
}
